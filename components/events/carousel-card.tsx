  ...
  return (
    <View className="flex-1 overflow-hidden rounded-3xl border border-border">
      ...
      <View className="absolute bottom-0 left-0 right-0 gap-3 p-6">
        ...
        <Text
          numberOfLines={2}
          className="text-center text-2xl font-primary-bold leading-8 text-foreground">
          {item.title}
        </Text>
        ...
        {categories.length > 0 && (
          <View className="flex-row flex-wrap justify-center gap-2">
            {categories.map((category, index) => (
              <View
                key={`${category}-${index}`}
                className="rounded-full border border-border bg-accent px-3 py-1">
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.85}
          className="mt-2 self-center rounded-full bg-primary px-6 py-2.5">
